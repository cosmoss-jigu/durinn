...
void lock_modeling(int64_t& sema, int64_t& lock) {
  while (!CAS(&sema, &lock, lock+1)) {
    lock = sema;
  }
}

void unlock_modeling(int64_t& sema, int64_t& lock) {
  while (!CAS(&sema, &lock, lock-1)) {
    lock = sema;
  }
}

bool lock_modeling1(int64_t& sema, int64_t& lock) {
  return CAS(&sema, &lock, -1);
}

void unlock_modeling1(int64_t& sema) {
  sema = 0;
}
...
int Segment::Insert(Key_t& key, Value_t value, size_t loc, size_t key_hash) {
  ...
  // in case shift overflow
  size_t hash_pattern;
  if (local_depth == 0)
    hash_pattern = 0;
  else
    hash_pattern = (key_hash >> (8*sizeof(key_hash)-local_depth));
  if (hash_pattern != pattern) return 2;
  auto lock = sema;
  int ret = 1;
  lock_modeling(sema, lock);
  /*
  while (!CAS(&sema, &lock, lock+1)) {
    lock = sema;
  }
  */
  ...
  for (unsigned i = 0; i < kNumPairPerCacheLine * kNumCacheLine; ++i) {
    ...
    if (local_depth == 0)
      ...
      hash_pattern = (h(&_[slot].key,sizeof(Key_t)) >> (8*sizeof(key_hash)-local_depth));
    if (hash_pattern != pattern) {
...
Segment** Segment::Split(void) {
  ...
  if (!lock_modeling1(sema, lock)) return nullptr;
...
RETRY:
  ...
  if (ret == 1) {
    ...
    // in case shift overflow
    size_t pattern_tmp;
    if (s[0]->local_depth <= 1)
      pattern_tmp = 0;
    else
      pattern_tmp = (key_hash >> (8*sizeof(key_hash)-s[0]->local_depth+1));
    s[0]->pattern = pattern_tmp << 1;

    // s[1]->pattern = ((key_hash >> (8*sizeof(key_hash)-s[1]->local_depth+1)) << 1) + 1;
    // in case shift overflow
    if (s[1]->local_depth <= 1)
      pattern_tmp = 0;
    else
      pattern_tmp = (key_hash >> (8*sizeof(key_hash)-s[1]->local_depth+1));
    s[1]->pattern = (pattern_tmp << 1) + 1;
    ...
    // Insert(key, value);
    goto STARTOVER;
    // TODO: if it is inconsistent it may try it again and again
  } else {
...
Value_t CCEH::Get(Key_t& key) {
  ...
  lock_modeling(dir->_[x]->sema, sema);
  /*
...
#ifdef INPLACE
      sema = dir->_[x]->sema;
      unlock_modeling(dir->_[x]->sema, sema);
      /*
      while (!CAS(&dir->_[x]->sema, &sema, sema-1)) {
        sema = dir->_[x]->sema;
      }
...
double CCEH::Utilization(void) {
  ...
  for (auto& elem: set) {
    for (unsigned i = 0; i < Segment::kNumSlot; ++i) {
      ...
      // in case shift overflow
      size_t hash_pattern;
      if (elem.first->local_depth== 0)
        hash_pattern = 0;
      else
        hash_pattern = key_hash >> (8*sizeof(key_hash)-elem.first->local_depth);
      if (hash_pattern == elem.first->pattern) sum++;
#else
                       ...
                       char *layout_name)
{
    ...
    if (is_created == 0) {
      printf("Reading from an existing cceh.\n");
      CCEH* ret = (CCEH*) root_obj->cceh_ptr;
      ret->Recovery();
      return ret;
    }
    CCEH *cceh = new CCEH();

    root_obj->cceh_ptr = cceh;
    clflush((char *)root_obj, sizeof(root_obj));
    return cceh;
}
