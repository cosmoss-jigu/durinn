...
#include "include/BSTAravindTraverse-ciri.h"
...
void run_op(char **op, BSTAravindTraverse<int>* list, FILE *output_file) {
  int key;
  int value;
  bool ret;
  switch (op[0][0]) {
    case 'i':
      ...
      api_begin();
      ret = list->insert(getpid(), key, value);
      api_end();
    ...
    case 'g':
      ...
      api_begin();
      value = list->contains(getpid(), key);
      api_end();
...
void *run_op_in_parallel(void* ptr) {
  ...
  args->list->bst_init_local();
  run_op(args->op, args->list, args->output_file);
  return 0;
}

void run_ops_in_parallel(char* ops[][3], BSTAravindTraverse<int>* list, FILE *output_file) {
...
int main(int argc, char *argv[]) {
  ...
  BSTAravindTraverse<int>* list = init_bst(pmem_path, pmem_size_in_mib, layout_name);
